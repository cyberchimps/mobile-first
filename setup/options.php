...
<label class="description" for="mobilefirst_options[slider]"><?php _e( 'Slidedeck Slider', 'mobile-first' ); ?> <em>(<?php printf(__( 'important: you must create a slider first after installing %1$sSlidedeck Slider%2$s - for best results set slider images width at your max display width - all slides need to be consistent in size.', 'mobile-first' ), '<a href="plugin-install.php?tab=search&s=SlideDeck+–+Responsive+WordPress+Slider+Plugin" target="_blank">', '</a>' ); ?>)</em></label>
<br />
...
<?php
?>
<tr valign="top"><th scope="row"><?php _e( 'Slider Shortcode', 'mobile-first' ); ?><br /><em>(<?php _e( 'Please enter the Shortcode Generated by Slider plugin here', 'mobile-first' ); ?>)</em></th>
<td>
<p class="mobilefirst-options-slider_shortcode">
<label class="description" for="mobilefirst_options[slider_shortcode]"><?php _e( 'Slider Shortcode', 'mobile-first' ); ?></label><br />
<input id="mobilefirst_options[slider_shortcode]" class="logo-upload-url" type="text" name="mobilefirst_options[slider_shortcode]" value="<?php echo esc_attr( $options['slider_shortcode'] ); ?>" />
</p>
</td>
</tr>
<?php
...
$valid_input['slider_shortcode'] = ( isset( $input['slider_shortcode'] ) ? esc_html( $input['slider_shortcode'] ) : $valid_input['slider_shortcode'] );
